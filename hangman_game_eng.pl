...
/*Predicato che gestisce la fase di gioco */
gioca(ParolaSegreta, TentativoAttuale, TentativiRimanenti) :-
    (   TentativiRimanenti =:= 0 ->
        ...
        write('La parola segreta era: '),stampa_lista(ParolaSegreta), nl
    ...
    ;   pulisciConsole,
        ...
        write('Inserisci una lettera: '), nl,
        get_char(Lettera),
        get_char(_),
        aggiornaGioco(ParolaSegreta, TentativoAttuale, Lettera,TentativiRimanenti)
    ).
...
aggiornaGioco(ParolaSegreta, TentativoAttuale, Lettera, TentativiRimanenti) :-
   ...
   ;   memberchk(Lettera, ParolaSegreta) ->
           ...
           append(TentativoAttuale, [Lettera], NuovoTentativo),
   ...
   ;   
       ...
       gioca(ParolaSegreta, TentativoAttuale, NuoviTentativi)
    ).


/*Cicla ricorsivamente la parola che gli viene passata e controlla se nella lista tentativo il carattere è presente,
se lo è lo stampa, altrimenti stampa un _ */
visualizzaParola([], _).
